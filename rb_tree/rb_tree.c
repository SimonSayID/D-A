    ...
    if (node != NULL) {
        ...
        if (temp != NULL) {
            ...
            while (dbn != NULL && dbn->color == '2') {
                ...
                if (rb_is_black(dbn->parent) && rb_is_red(sibling)
                ...
                if (rb_is_black(dbn->parent) && rb_is_black(sibling) && is_nephews_both_black(sibling)) {
                ...
                sibling = dbn->parent->left == dbn ? dbn->parent->right : dbn->parent->left;
                if (sibling != NULL) {
                    if (rb_is_red(dbn->parent) && rb_is_black(sibling)
                        && is_nephews_both_black(sibling)) {
                        rb_color_red(sibling);
                        rb_color_black(dbn->parent);
                        if (dbn->parent->left == dbn) {
                ...
                if (sibling != NULL) {
                    if (rb_is_black(sibling)) {
                        if (dbn->parent->left == dbn
                            && sibling->left != NULL && rb_is_red(sibling->left)
                            && (sibling->right == NULL || rb_is_black(sibling->right))) {

                            dbn->parent->right = rb_right_rotation(dbn->parent->right);

                        } else if (dbn->parent->right == dbn
                                   && sibling->right != NULL && rb_is_red(sibling->right)
                                   && (sibling->left == NULL || rb_is_black(sibling->left))) {
